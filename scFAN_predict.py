...
python scFAN_predict.py -i /data2/fly/PBMCs/raw_data/gz_files/new_folder/LMPP -scindir /data2/fly/scFAN_data/new_folder_cisTopics_PBMC_agg -moname LMPP -pb True -oc -usea True -oc multiTask_H1hESC_add_ATAC_moreTFs multiTask_GM12878_add_ATAC_moreTFs multiTask_K562_ATAC_more_chipdata
"""
...
def make_argument_parser():
    ...
    parser.add_argument('--using_agg', '-usea', type=ast.literal_eval,
                        help='whether using aggregate data of single cell, default is true')
...
def main():
    ...
    for num_pos,positive_windows in enumerate(positive_windows_list):
        _, datagen_bed = utils.load_bed_data_sc(genome, positive_windows, False, False, input_dir, False, big_wig_list,num_pos,input_scATAC_dir,using_agg, chrom=None)
        #pdb.set_trace()
        
        print "%d sample...in %d "%(num_pos,len(positive_windows_list))
        model_predicts_list,model_tfs_list = test(output_dir,datagen_bed)
        try:
            #os.stat('%s/deepATAC_fea_scbw_moreTFs_only_sc_neighbor_cis_db_K562_50'%(input_dirs[0]))
            os.stat('%s/scFAN_predict_using_H1'%(input_dirs[0]))
        except:
            #os.mkdir('%s/deepATAC_fea_scbw_moreTFs_only_sc_neighbor_cis_db_K562_50'%(input_dirs[0]))
            ...
            #os.mkdir('%s/deepATAC_fea_scbw_moreTFs_only_sc_neighbor_cis_db_onlysc_H1'%(input_dirs[0]))
            #os.mkdir('%s/deepATAC_fea_scbw_moreTFs_only_sc_neighbor_cis_db_H1_50a'%(input_dirs[0]))
            #os.mkdir('%s/deepATAC_fea_scbw_moreTFs_only_sc_neighbor_cis_db_onlysc_GM'%(input_dirs[0]))
            #os.mkdir('%s/deepATAC_fea_scbw_moreTFs_only_sc_neighbor_cis_db_GM_50a'%(input_dirs[0]))
        ...
        #np.save('%s/deepATAC_fea_scbw_moreTFs_only_sc_neighbor_cis_db_onlysc_H1/%s_data'%(input_dirs[0],str(num_pos)),model_predicts_list[0])
        #np.save('%s/deepATAC_fea_scbw_moreTFs_only_sc_neighbor_cis_db_GM_50a/%s_data'%(input_dirs[0],str(num_pos)),model_predicts2_list[1])
        #np.save('%s/deepATAC_fea_scbw_moreTFs_only_sc_neighbor_cis_db_onlysc_GM/%s_data'%(input_dirs[0],str(num_pos)),model_predicts_list[1])
        #model_predicts_list.append(model_predicts) 
        for index in range(model_predicts_list[0].shape[0]):
            ...
            #test_predict_all_TFs2 = test_predict_all_TFs2 + [model_tfs2_list[0][item] for item in np.argsort(model_predicts2_list[0][index])[::-1][:2]]
            ...
            #test_predict_all_TFs_GM_50 = test_predict_all_TFs_GM_50 + [model_tfs2_list[1][item] for item in np.argsort(model_predicts2_list[1][index])[::-1][:2]]
        ...
        #bb2.append([Counter(test_predict_all_TFs2)[item]/float(sum(Counter(test_predict_all_TFs2).values())) for item in model_tfs2_list[0]])
        ...
        #bb2_GM_50.append([Counter(test_predict_all_TFs_GM_50)[item]/float(sum(Counter(test_predict_all_TFs_GM_50).values())) for item in model_tfs2_list[1]])
    ...
    #bb3 = np.array(bb3)
    #bb2_GM_50 = np.array(bb2_GM_50)
    #bb3_GM_sc = np.array(bb3_GM_sc)
    ...
    #np.save('%s/count_motif_scbw_new_dis_thres_2_%s_moreTFs_only_sc_neighbor_cis_db_K562_50'%(input_dirs[0],motif_name),bb3)
    ...
    #np.save('%s/count_motif_scbw_new_dis_thres_2_%s_moreTFs_only_sc_neighbor_cis_db_onlysc_H1'%(input_dirs[0],motif_name),bb3)
    #np.save('%s/count_motif_scbw_new_dis_thres_2_%s_moreTFs_only_sc_neighbor_cis_db_H1_50a'%(input_dirs[0],motif_name),bb2)
    #np.save('%s/count_motif_scbw_new_dis_thres_2_%s_moreTFs_only_sc_neighbor_cis_db_onlysc_GM'%(input_dirs[0],motif_name),bb3_GM_sc)
    #np.save('%s/count_motif_scbw_new_dis_thres_2_%s_moreTFs_only_sc_neighbor_cis_db_GM_50a'%(input_dirs[0],motif_name),bb2_GM_50)
    ...
    #t1 = commands.getoutput('rm -f %s/deepATAC_fea_scbw_moreTFs_only_sc_neighbor_cis_db_H1_50a/*.npy'%(input_dirs[0]))
    #t2 = commands.getoutput('rm -f %s/deepATAC_fea_scbw_moreTFs_only_sc_neighbor_cis_db_onlysc_H1/*.npy'%(input_dirs[0]))
    #t3 = commands.getoutput('rm -f %s/deepATAC_fea_scbw_moreTFs_only_sc_neighbor_cis_db_GM_50a/*.npy'%(input_dirs[0]))
    #t4 = commands.getoutput('rm -f %s/deepATAC_fea_scbw_moreTFs_only_sc_neighbor_cis_db_onlysc_GM/*.npy'%(input_dirs[0]))
