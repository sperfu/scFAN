...
def sort_find_neighbors(sim_all,cell_name_list,k_neighbor=50):
    ...
    for index,item in enumerate(cell_name_list):
        ...
        aa = subprocess.getstatusoutput('mkdir bedgraph_PBMC')
        aa = subprocess.getstatusoutput('bigWigMerge '+sel_name_list+' bedgraph_PBMC/'+cell_name_list[index]+'.bedGraph')
        cc = subprocess.getstatusoutput('sort -k1,1 -k2,2n '+'bedgraph_PBMC/'+cell_name_list[index]+'.bedGraph > bedgraph_PBMC/'+cell_name_list[index]+'.bedgraph')
        dd = subprocess.getstatusoutput('rm -f bedgraph_PBMC/'+cell_name_list[index]+'.bedGraph')
        dd = subprocess.getstatusoutput('mkdir new_folder_cisTopics_PBMC_agg')
        bb = subprocess.getstatusoutput('bedGraphToBigWig bedgraph_PBMC/'+cell_name_list[index]+'.bedgraph'+' /data2/fly/fly_scFAN_bak_20190603/resources/hg19.autoX.chrom.sizes new_folder_cisTopics_PBMC_agg/'+cell_name_list[index]+'.bw')
        if index%100 == 0:
