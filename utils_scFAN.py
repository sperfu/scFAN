...
def load_chip_multiTask_multiple(input_dir):
    tfs, chip_beds_list, merged_chip_bed_list = get_chip_beds_multiple(input_dir)
    ...
    for index,merged_chip_bed in enumerate(merged_chip_bed_list):
        ...
        y_positive = parmap.map(intersect_count, [chip_beds_list[index]], positive_windows.fn)
...
def load_bigwigs_sc(input_dirs,num_pos,bigwig_lists):
    bigwig_names = None
    bigwig_files_list = []
    for input_dir in input_dirs:
...
def load_bed_data_sc(genome, positive_windows, use_meta, use_gencode, input_dir, is_sorted, big_wig_list, num_pos, chrom=None):
    ...
    print 'Generating test data iterator'
    bigwig_names, bigwig_files_list = load_bigwigs_sc([input_dir],num_pos,big_wig_list)
    bigwig_files = bigwig_files_list[0]
    if use_meta:
        meta_names, meta_list = load_meta([input_dir])
        meta = meta_list[0]
    ...
    bigwig_rc_order = get_bigwig_rc_order(bigwig_names)
    datagen_bed = DataIterator(data_bed, genome, 100, L, bigwig_rc_order, shuffle=False)
    return bigwig_names, meta_names, datagen_bed
