...
pybedtools.set_tempdir('/data2/fly/tmp')
batch_size = 100
...
def load_bigwigs_sc(input_dirs,num_pos,bigwig_lists,input_scATAC_dir,using_agg):
    bigwig_names = None
    bigwig_files_list = []
    for input_dir in input_dirs:
        ...
        if not os.path.isfile(input_bigwig_info_file):
            ...
            ### change it to newly added bw files in /data2/fly/scFAN_data
            #input_bigwig_files.append('/data2/fly/scFAN_data/new_folder_cisTopics_PBMC_50/'+bigwig_lists[num_pos])
            if using_agg:
                #input_bigwig_files.append('/data2/fly/scFAN_data/new_folder_cisTopics_PBMC_agg/'+bigwig_lists[num_pos])
                input_bigwig_files.append(input_scATAC_dir[0]+'/'+bigwig_lists[num_pos])
            else:
                #input_bigwig_files.append('/data2/fly/scFAN_data/new_folder_PBMC_ori/'+bigwig_lists[num_pos])
                input_bigwig_files.append(input_scATAC_dir[0]+'/'+bigwig_lists[num_pos])
...
def load_bed_data_sc(genome, positive_windows, use_meta, use_gencode, input_dir, is_sorted, big_wig_list, num_pos, input_scATAC_dir, using_agg, chrom=None):
    bed_filtered = positive_windows
    print 'Generating test data iterator'
    bigwig_names, bigwig_files_list  = load_bigwigs_sc([input_dir],num_pos,big_wig_list,input_scATAC_dir,using_agg)
